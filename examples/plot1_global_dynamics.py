import sys
import os
import numpy as np
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
from scipy.integrate import solve_ivp

# Ensure Python can look backward and find the 'aoad' folder
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from aoad.dynamics import aoad_vector_field, compute_commutator_energy


def main():
    n = 3
    alpha, beta = 1.2, 0.8
    np.random.seed(42)

    A, B = np.random.randn(n, n), np.random.randn(n, n)
    L_G0 = 0.5 * (A + A.T)
    L_J0 = 0.5 * (B + B.T)

    t_span = (0, 15.0)
    t_eval = np.linspace(t_span[0], t_span[1], 1000)
    y0 = np.concatenate([L_G0.flatten(), L_J0.flatten()])

    sol = solve_ivp(
        aoad_vector_field,
        t_span,
        y0,
        args=(n, alpha, beta),
        t_eval=t_eval,
        rtol=1e-10,
        atol=1e-13,
    )

    energies = []
    for i in range(len(sol.t)):
        LG = sol.y[0 : n * n, i].reshape((n, n))
        LJ = sol.y[n * n :, i].reshape((n, n))
        energies.append(compute_commutator_energy(LG, LJ))

    plt.figure(figsize=(7, 4.5))
    plt.plot(sol.t, energies, 'b-', linewidth=2, label='Empirical Energy $E(t)$')
    plt.title('Figure 1: Global Non-linear Relaxation to Commuting Variety')
    plt.xlabel('Time ($t$)')
    plt.ylabel('Commutator Energy')
    plt.grid(True, ls='--')
    plt.legend()
    plt.tight_layout()
    plt.savefig('plot1_global_dynamics.png', dpi=150, bbox_inches='tight')
    print('Saved plot1_global_dynamics.png')


if __name__ == '__main__':
    main()
